...
async function main() {
  console.log(`\n${colors.orange}${colors.bright}⚡ Yunque Bots (Forja OS)${colors.reset} ${colors.gray}· Plataforma de Chatbots de IA para Empresas${colors.reset}\n`);
...
async function runInit() {
  ...
  if (!isNonInteractive) {
    ...
    console.log(`${colors.gold}Asistente de Configuración Inicial:${colors.reset}\n`);
    ...
    console.log(`  1. ${colors.bright}Google Gemini 3.5 Flash Lite${colors.reset} (gemini-3.5-flash-lite) - ${colors.green}Ultrarrápido, económico y preciso${colors.reset}`);
  ...
  console.log(`\n${colors.green}✓${colors.reset} ${colors.bright}Bot configurado exitosamente!${colors.reset}`);
  ...
  console.log(`  1. Inicia el panel y simulador local con:  ${colors.cyan}npm run dev${colors.reset}  (o  ${colors.cyan}node bin/yunque.js serve${colors.reset})`);
...
// ================= COMMAND: LIST =================
function runList() {
  console.log(`${colors.gold}Catálogo de Plantillas y Giros de Negocio Disponibles:${colors.reset}\n`);
...
async function runInstall(slug) {
  if (!slug) {
    console.log(`${colors.red}Falta el slug del giro.${colors.reset} Usa: ${colors.cyan}node bin/yunque.js install <slug>${colors.reset} o consulta ${colors.cyan}node bin/yunque.js list${colors.reset}`);
    return;
  }
...
async function runDoctor() {
  console.log(`${colors.gold}Diagnóstico de Salud del Sistema (Doctor):${colors.reset}\n`);
  let issues = 0;
  ...
  if (config.bot?.name) {
    ...
    console.log(`  ${colors.red}✗${colors.reset} Configuración no encontrada. Corre ${colors.cyan}node bin/yunque.js init${colors.reset}`);
    issues++;
  }
  ...
  if (issues === 0) {
    console.log(`\n${colors.green}${colors.bright}Todo en orden. Tu bot está listo para operar.${colors.reset}\n`);
  } else {
...
async function runUpdate() {
  console.log(`${colors.gold}Iniciando actualización segura del bot...${colors.reset}\n`);
...
async function runDeploy() {
  console.log(`${colors.gold}${colors.bright}🚀 Centro de Despliegue y Publicación a Producción${colors.reset}\n`);
  ...
  console.log(`     • Iniciar servicio:       ${colors.orange}pm2 start src/server/index.js --name "yunque-bot"${colors.reset}`);
  console.log(`     • Guardar reinicio:       ${colors.orange}pm2 save && pm2 startup${colors.reset}\n`);
}

function printHelp() {
  console.log(`Uso: ${colors.cyan}node bin/yunque.js <comando> [opciones]${colors.reset}\n`);
...
main().catch(err => {
  console.error(`${colors.red}Error ejecutando CLI:${colors.reset}`, err);
  process.exit(1);
});
